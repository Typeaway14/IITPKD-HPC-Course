...
int main(int argc, char *argv[]){
	...
	if(rank==0){
		...
		psdata1=(int *)malloc(local_size*sizeof(int));
		psdata2=(int *)malloc(local_size*sizeof(int));
			for(int i=0;i<local_size;i++){ 
				psdata1[i]=sorted_data[i]; psdata2[i]=sorted_data[i+local_size]; 
			}
		   merge(sorted_data, psdata1,local_size, psdata2, local_size); 
		}
